...
export const createInvestmentProposal = onCall<InvestmentProposalData>(async (request) => {
  if (!request.auth) {
    throw new Error("User must be authenticated to create investment proposals.");
  }
  ...
  try {
    ...
    const investorDoc = await db.collection("users").doc(request.auth.uid).get();
    ...
    const businessIdeaDoc = await db.collection("businessIdeas").doc(businessIdeaId).get();
    ...
    const proposalRef = await db.collection("investmentProposals").add({
      ...
      investorName: investorProfile.displayName || investorProfile.profile?.fullName,
      investorEmail: investorProfile.email,
      investorProfile: investorProfile.isComplete
        ? {
            uid: request.auth.uid,
            fullName: investorProfile.profile?.fullName || investorProfile.displayName,
    ...
    throw error instanceof Error ? error : new Error("Failed to create investment proposal.");
  }
});

// Accept investment proposal
export const acceptInvestmentProposal = onCall<AcceptInvestmentData>(async (request) => {
  if (!request.auth) {
    throw new Error("User must be authenticated to accept investment proposals.");
  }
  ...
  try {
    ...
    const proposalDoc = await db.collection("investmentProposals").doc(proposalId).get();
    ...
    throw error instanceof Error ? error : new Error("Failed to accept investment proposal.");
  }
});
