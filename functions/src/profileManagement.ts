...
export const completeUserProfile = https.onCall(
  { cors: true },
  async (request) => {
    try {
      ...
      const { profileData }: { profileData: CompleteProfileData } =
        request.data;
      ...
      if (!profileData.fullName?.trim()) {
        throw new https.HttpsError("invalid-argument", "Full name is required");
      }
      ...
      throw new https.HttpsError("internal", "Failed to complete profile");
    }
  },
);

// Get user profile completion status
export const getProfileCompletionStatus = https.onCall(
  { cors: true },
  async (request) => {
    try {
      // Verify authentication
      ...
      throw new https.HttpsError("internal", "Failed to get profile status");
    }
  },
);
